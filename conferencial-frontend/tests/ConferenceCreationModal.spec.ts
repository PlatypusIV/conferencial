...
test.describe("ConferenceCreationModal", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.locator(".calendarConferenceListContainer").first().click();
  });
  ...
  test("should render the modal with form fields", async ({ page }) => {
    ...
    await expect(
      modal.locator('input[placeholder="Example conference"]')
    ).toBeVisible();
    await expect(modal.getByLabel("duration")).toBeVisible();
    await expect(
      modal.locator('input[placeholder="Start time"]')
    ).toBeVisible();
  ...
  test("should show validation messages on empty submit", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Please input conference name")
    ).toBeVisible();
    await expect(
      page.locator("text=Please input conference duration")
    ).toBeVisible();
